...
Для непосредственно запросов устанавливается множество скриптов:
...
База развертывается с помощью команды 4s-boss(это стоило часов мучения с компиляцией из исходников):

![start](./assets/start.png)

Примеры запросов на тестовых данных(данные см. ниже):

```SPARQL
SELECT DISTINCT * WHERE { GRAPH ?G { ?s ?p ?o } } ORDER BY ?s ?p ?o
```

```SPARQL
INSERT DATA { GRAPH <a> { <x> <y> <z> . <a> <b> <c> . } }
```

```SPARQL
DELETE DATA { GRAPH <a> { <a> <b> <d> . } }
```
...
Данные для демонстрации можно найти, например, здесь: 
1. Данные: <https://github.com/4store/4store/tree/master/tests/query/exemplar>
2. Запрос: <https://github.com/4store/4store/blob/master/tests/query/scripts/>
3. Скрипт для запуска(требуется Perl): <https://github.com/4store/4store/blob/master/tests/query/run.pl>
...
Демоданные можно найти в директории с тестами: <https://github.com/4store/4store/tree/master/tests/query>.
